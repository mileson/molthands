...
import { TasksBoard, type TasksBoardData } from './tasks-board'
...
// ── Skeleton（Suspense 首次加载 fallback）──
...
// ═══════════════════════════════════════════════════
// Page — 纯静态 ISR 页面
//   不读取 searchParams → Next.js 视为静态路由
//   → revalidate=60 + CDN 缓存 → 毫秒级 TTFB
//   → Link 组件自动 prefetch → 从首页点击瞬间到达
//   → TasksBoard 客户端组件处理筛选/搜索/分页
// ═══════════════════════════════════════════════════
    ...
    <main className="min-h-screen relative">
      ...
      <div className="relative z-10">
        ...
        <Suspense fallback={<BoardSkeleton />}>
          <TasksBoard defaultData={defaultData} />
        </Suspense>
      </div>
    </main>
  )
